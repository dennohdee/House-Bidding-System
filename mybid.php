<?php
 ...
 include "conn.php";
    ...
    //fetch my bids
    $uid = $_SESSION['uid'];
    $sql3 = "SELECT `bid`.`bidprice`,`bid`.`time`,`bid`.`status`,`house`.`type`,`userdetails`.`surname`,`userdetails`.`othernames`,`userdetails`.`email`,`userdetails`.`phone` FROM `bid`,`house`,`userdetails` WHERE `house`.`hid`=`bid`.`hid` AND `userdetails`.`uid`=`house`.`uid` AND `bid`.`uid`='$uid'";
